...
#[cfg(test)]
pub mod tests {
    use crate::conversion::{oeb::{OebBook, OebChapter}, epub_builder};
    ...
    fn test_epub_builder_roundtrip() {
        ...
        assert!(archive.by_name("META-INF/container.xml").is_ok(), "container.xml missing");
        assert!(archive.by_name("OEBPS/content.opf").is_ok(), "content.opf missing");
        assert!(archive.by_name("OEBPS/nav.xhtml").is_ok(), "nav.xhtml missing");
        assert!(archive.by_name("OEBPS/toc.ncx").is_ok(), "toc.ncx missing");
        assert!(archive.by_name("OEBPS/Text/chapter_001.xhtml").is_ok(), "chapter_001.xhtml missing");
    ...
    async fn test_unsupported_format_error() {
        let result = crate::conversion::convert_to_epub_new(
            std::path::Path::new("test.xyz"),
            None,
        ).await;
        assert!(
            matches!(result, Err(crate::conversion::ConversionError::UnsupportedFormat(_))),
