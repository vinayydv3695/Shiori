    ...
    while let Some(c) = chars.next() {
        if let Some(end_tag) = skip_until {
            ...
            if c == '<' {
                ...
                let rest: String = std::iter::once(c).chain(chars.clone().take(end_tag.len() + 2)).collect();
                let rest_lower = rest.to_lowercase();
                if rest_lower.contains(&format!("/{}>", &end_tag[1..])) || rest_lower.contains(&format!("/{} ", &end_tag[1..])) {
                    skip_until = None;
...
pub fn sanitize_html_for_epub(html: &str) -> String {
    ...
    let drop_tag_only: &[&str] = &["html", "body", "div", "span", "font",
                                    "center", "article", "section", "aside",
                                    "header", "footer", "nav", "main", "figure"];
    ...
    while let Some(c) = chars.next() {
        ...
        if c == '<' {
            ...
            for _ in 0..3 { if let Some(cc) = tmp.next() { peeked.push(cc); } }
        ...
        for cc in chars.by_ref() {
            if cc == '>' { closed_gt = true; break; }
            tag_buf.push(cc);
        }
        if !closed_gt { continue; } // malformed, skip
        ...
        let tag_name_raw: String = name_part.chars()
        ...
        if allowed_tags.contains(&tag_name.as_str()) {
            if is_closing {
                ...
                if tag_name == "br" || tag_name == "hr" { continue; }
...
fn extract_safe_attrs(tag_buf: &str, tag_name: &str) -> String {
    ...
    let attr_re = regex::Regex::new(r#"(\w[\w:\-]*)(?:\s*=\s*(?:"([^"]*)"|'([^']*)'|(\S+)))?"#).unwrap();

    for cap in attr_re.captures_iter(tag_buf) {
        let key = cap.get(1).map_or("", |m| m.as_str()).to_lowercase();
        let val = cap.get(2).or_else(|| cap.get(3)).or_else(|| cap.get(4))
                    .map_or("", |m| m.as_str());
        ...
        if keep && !val.is_empty() {
            out.push_str(&format!(" {}=\"{}\"", key, super::epub_writer::escape_xml(val)));
        }
    }
    out
}
        ...
        if let Some(semi_rel) = window.find(';') {
            ...
            let decoded: Option<char> = if entity.starts_with('#') {
                ...
                if num_str.starts_with('x') || num_str.starts_with('X') {
                    u32::from_str_radix(&num_str[1..], 16).ok().and_then(char::from_u32)
                } else {
...
fn named_entity(name: &str) -> Option<char> {
    Some(match name {
        ...
        "equiv"            => '\u{2261}', // ≡
        "not"              => '\u{00AC}', // ¬
        "oelig" | "OElig"  => if name.starts_with('O') { '\u{0152}' } else { '\u{0153}' }, // Œ/œ
        "fnof"             => '\u{0192}', // ƒ
        "Alpha" => '\u{0391}', "alpha" => '\u{03B1}',
        "Beta"  => '\u{0392}', "beta"  => '\u{03B2}',
        "Gamma" => '\u{0393}', "gamma" => '\u{03B3}',
        "Delta" => '\u{0394}', "delta" => '\u{03B4}',
        "Epsilon" => '\u{0395}', "epsilon" | "epsiv" => '\u{03B5}',
        "Zeta"  => '\u{0396}', "zeta"  => '\u{03B6}',
        "Eta"   => '\u{0397}', "eta"   => '\u{03B7}',
        "Theta" => '\u{0398}', "theta" | "thetav" | "vartheta" => '\u{03B8}',
        "Iota"  => '\u{0399}', "iota"  => '\u{03B9}',
        "Kappa" => '\u{039A}', "kappa" | "kappav" => '\u{03BA}',
        "Lambda" => '\u{039B}', "lambda" => '\u{03BB}',
        "Mu"    => '\u{039C}', "mu"    => '\u{03BC}',
        "Nu"    => '\u{039D}', "nu"    => '\u{03BD}',
        "Xi"    => '\u{039E}', "xi"    => '\u{03BE}',
        "Omicron" => '\u{039F}', "omicron" => '\u{03BF}',
        "Pi"    => '\u{03A0}', "pi" | "piv" => '\u{03C0}',
        "Rho"   => '\u{03A1}', "rho" | "rhov" | "varrho" => '\u{03C1}',
        "Sigma" => '\u{03A3}', "sigma" | "sigmav" => '\u{03C3}',
        "Tau"   => '\u{03A4}', "tau"   => '\u{03C4}',
        "Upsilon" => '\u{03A5}', "upsilon" => '\u{03C5}',
        "Phi"   => '\u{03A6}', "phi" | "phiv" | "varphi" => '\u{03C6}',
        ...
        "iexcl"  => '\u{00A1}', "cent"   => '\u{00A2}',
        "pound"  => '\u{00A3}', "curren" => '\u{00A4}',
        "yen"    => '\u{00A5}', "brvbar" => '\u{00A6}',
        "sect"   => '\u{00A7}', "uml"    => '\u{00A8}',
        "ordf"   => '\u{00AA}',
        "macr"   => '\u{00AF}', "acute"  => '\u{00B4}',
        "ordm"   => '\u{00BA}',
        "iquest" => '\u{00BF}',
        "Agrave" => '\u{00C0}', "Aacute" => '\u{00C1}',
        "Acirc"  => '\u{00C2}', "Atilde" => '\u{00C3}',
        "Auml"   => '\u{00C4}', "Aring"  => '\u{00C5}',
        "AElig"  => '\u{00C6}', "Ccedil" => '\u{00C7}',
        "Egrave" => '\u{00C8}', "Eacute" => '\u{00C9}',
        "Ecirc"  => '\u{00CA}', "Euml"   => '\u{00CB}',
        "Igrave" => '\u{00CC}', "Iacute" => '\u{00CD}',
        "Icirc"  => '\u{00CE}', "Iuml"   => '\u{00CF}',
        "ETH"    => '\u{00D0}', "Ntilde" => '\u{00D1}',
        "Ograve" => '\u{00D2}', "Oacute" => '\u{00D3}',
        "Ocirc"  => '\u{00D4}', "Otilde" => '\u{00D5}',
        "Ouml"   => '\u{00D6}', "Oslash" => '\u{00D8}',
        "Ugrave" => '\u{00D9}', "Uacute" => '\u{00DA}',
        "Ucirc"  => '\u{00DB}', "Uuml"   => '\u{00DC}',
        "Yacute" => '\u{00DD}', "THORN"  => '\u{00DE}',
        "szlig"  => '\u{00DF}',
        "agrave" => '\u{00E0}', "aacute" => '\u{00E1}',
        "acirc"  => '\u{00E2}', "atilde" => '\u{00E3}',
        "auml"   => '\u{00E4}', "aring"  => '\u{00E5}',
        "aelig"  => '\u{00E6}', "ccedil" => '\u{00E7}',
        "egrave" => '\u{00E8}', "eacute" => '\u{00E9}',
        "ecirc"  => '\u{00EA}', "euml"   => '\u{00EB}',
        "igrave" => '\u{00EC}', "iacute" => '\u{00ED}',
        "icirc"  => '\u{00EE}', "iuml"   => '\u{00EF}',
        "eth"    => '\u{00F0}', "ntilde" => '\u{00F1}',
        "ograve" => '\u{00F2}', "oacute" => '\u{00F3}',
        "ocirc"  => '\u{00F4}', "otilde" => '\u{00F5}',
        "ouml"   => '\u{00F6}', "oslash" => '\u{00F8}',
        "ugrave" => '\u{00F9}', "uacute" => '\u{00FA}',
        "ucirc"  => '\u{00FB}', "uuml"   => '\u{00FC}',
        "yacute" => '\u{00FD}', "thorn"  => '\u{00FE}',
        "yuml"   => '\u{00FF}',
...
pub fn looks_like_heading(text: &str) -> bool {
    ...
    // Keyword match
    let keywords = [
        "chapter", "part", "section", "book", "prologue", "epilogue",
        "introduction", "preface", "afterword", "appendix", "interlude",
        "act ", "scene ", "volume",
    ];
    ...
    if words.len() <= 2 {
        let first = words[0];
        let is_numeral = first.chars().all(|c| c.is_numeric() || c == '.' || c == ')')
