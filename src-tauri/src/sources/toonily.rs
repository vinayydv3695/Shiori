//! Toonily.com source plugin for Shiori.
//!
//! ManhwaHub (toonily.com) source plugin for Shiori
//! around toonily.com, which is a standard Madara-theme WordPress manga site.
//!
//! Two `Source` implementations are exposed:
//!  - `ToonilyManhwaSource`  – SFW manhwa (id: "toonily")
//!  - `ToonilyManhwa18Source` – NSFW manhwa18 (id: "toonily18")
//!
//! Both share the same parsing engine via `ToonilyEngine`.
...
impl ToonilyEngine {
    fn new(category: &'static str) -> Result<Self> {
        let client = reqwest::Client::builder()
            ...
            .cookie_store(true)
            .build()
            .map_err(|e| ShioriError::Other(format!("Failed to build Toonily client: {}", e)))?;
    ...
    async fn fetch(&self, url: &str) -> Result<String> {
        ...
        let resp = self
            ...
            .send()
            .await
            .map_err(|e| ShioriError::Other(format!("Toonily fetch failed for {}: {}", url, e)))?;

        if !resp.status().is_success() {
            return Err(ShioriError::Other(format!(
                "Toonily returned HTTP {} for {}",
                resp.status(),
                url
            )));
        }
        ...
        resp.text()
            .await
            .map_err(|e| ShioriError::Other(format!("Toonily read body failed: {}", e)))
    }
    ...
    async fn search(&self, query: &str, page: u32, source_id: &str) -> Result<Vec<SearchResult>> {
        ...
        if query.starts_with("http") && query.contains("toonily.com") {
            let html = match self.fetch(query).await {
                Ok(h) => h,
                Err(e) => {
                    eprintln!("Toonily direct URL fetch error: {}", e);
                    return Ok(vec![]);
                }
            };
    ...
    async fn get_pages(&self, chapter_id: &str) -> Result<Vec<Page>> {
        ...
        if pages.is_empty() {
            return Err(ShioriError::Other(
                "Toonily: no page images found. The chapter URL may be incorrect.".to_string(),
            ));
        }
...
/// Source for ManhwaHub (toonily.com) source plugin for Shiori.
pub struct ToonilySource {
    engine: ToonilyEngine,
}

impl ToonilySource {
    pub fn new() -> Result<Self> {
        Ok(Self {
            engine: ToonilyEngine::new("manhwa")?,
        })
    }
}

#[async_trait::async_trait]
impl Source for ToonilySource {
    ...
    fn meta(&self) -> SourceMeta {
        SourceMeta {
            id: "toonily".to_string(),
            name: "Toonily".to_string(),
    ...
    async fn search(&self, query: &str, page: u32) -> Result<Vec<SearchResult>> {
        self.engine.search(query, page, "toonily").await
    }

    async fn search_with_meta(&self, query: &str, page: u32, limit: u32) -> Result<SearchResponse> {
        let items = self.engine.search(query, page, "toonily").await?;
        Ok(SearchResponse {
            items,
            total: None,
            offset: None,
    ...
    async fn get_chapters(&self, content_id: &str) -> Result<Vec<Chapter>> {
        self.engine.get_chapters(content_id, "toonily").await
    }
