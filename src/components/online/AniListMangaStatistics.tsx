  ...
  return (
    <div className="flex flex-col gap-8 pb-12">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <MetricCard label="Total Manga" value={stats.count} icon="📖" />
        <MetricCard label="Chapters Read" value={stats.chaptersRead} icon="📚" />
        <MetricCard label="Days Read" value={estimatedDaysRead} icon="📅" />
        <MetricCard label="Mean Score" value={stats.meanScore.toFixed(2)} icon="⭐" />
        <MetricCard label="Standard Dev" value={stats.standardDeviation.toFixed(2)} icon="📈" />
      </div>
      ...
      <div className="space-y-4">
        ...
        <div className="flex flex-col gap-3 bg-secondary/10 p-4 rounded-xl border border-border/20">
          ...
          {stats.scores.slice().sort((a, b) => a.score - b.score).map((s) => {
...
function PieChartCard({ title, data, nameKey, valueKey }: { title: string, data: any[], nameKey: string, valueKey: string }) {
  ...
  // Sort data descending to make pie chart look nicer
  const sortedData = [...data].sort((a, b) => b[valueKey] - a[valueKey]);
  
  return (
    <div className="space-y-3">
      <h3 className="text-sm font-semibold text-muted-foreground uppercase tracking-wider">{title}</h3>
      <div className="bg-secondary/10 rounded-xl border border-border/20 p-4 flex flex-col md:flex-row items-center gap-6 h-[250px]">
        <div className="h-full w-full max-w-[200px] flex-1 relative drop-shadow-xl">
          <ResponsiveContainer width="100%" height="100%">
            <PieChart>
              <defs>
                <radialGradient id="pieGradient" cx="50%" cy="50%" r="50%" fx="30%" fy="30%">
        ...
        <div className="flex-1 flex flex-col gap-2 w-full">
          {sortedData.slice(0, 5).map((item, i) => (
            <div key={i} className="flex items-center justify-between text-sm">
              <span className="font-medium truncate pr-4 text-muted-foreground capitalize">
                {item[nameKey].toLowerCase().replace(/_/g, ' ')}
              </span>
              <span className="font-semibold text-foreground">{item[valueKey]}</span>
            </div>
          ))}
        </div>
      </div>
