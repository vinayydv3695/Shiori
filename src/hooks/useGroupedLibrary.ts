export function tryExtractSeries(title: string): string | null {
...
export function extractSeriesTitle(book: Book): string | null {
  return book.series?.trim() || tryExtractSeries(book.title);
}
...
import { compareBooksNatural } from '@/lib/seriesSorting'
...
export function groupBooksBySeries(
  ...
  for (let i = 0; i < books.length; i++) {
    const book = books[i]
    const series = extractSeriesTitle(book)
    if (series) {
  ...
  const sortedSeries = Array.from(seriesMap.entries()).sort((a, b) => collator.compare(a[0], b[0]))
  for (let i = 0; i < sortedSeries.length; i++) {
    ...
    seriesBooks.sort((a, b) => compareBooksNatural(a, b, 'chapter_asc'))
...
export function useGroupedLibrary(
  ...
  return useMemo(() => groupBooksBySeries(books, enableGrouping), [books, enableGrouping])
}
