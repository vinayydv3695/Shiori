...
// In-memory cache for book cover lookups to prevent duplicate network calls & 429 rate limiting
...
export async function fetchCoverForBook(title: string, author?: string): Promise<string | null> {
  ...
  const cacheKey = `${cleanTitle.toLowerCase()}:${cleanAuthor.toLowerCase()}`;

  if (coverCache.has(cacheKey)) {
    return coverCache.get(cacheKey) || null;
  }

  // 1. Try Google Books API first (if not currently rate limited)
  if (Date.now() > googleBooksRateLimitedUntil) {
    try {
      ...
      if (gRes.status === 429) {
        // Back off Google Books API calls for 60 seconds on rate limit
        googleBooksRateLimitedUntil = Date.now() + 60000;
      } else if (gRes.ok) {
  ...
  try {
    ...
    if (response.ok) {
      ...
      if (data.docs && data.docs.length > 0 && data.docs[0].cover_i) {
        const cover = `https://covers.openlibrary.org/b/id/${data.docs[0].cover_i}-L.jpg`;
        coverCache.set(cacheKey, cover);
        return cover;
      }
    }
